...
podTemplate(label: worker, containers: [
  ...
  containerTemplate(name: 'e2e', image: '303679435142.dkr.ecr.eu-west-1.amazonaws.com/springboot-e2e:1.0.1', ttyEnabled: true),
  containerTemplate(name: 'kubectl', image: 'lachlanevenson/k8s-kubectl:v1.14.1', command: 'cat', ttyEnabled: true),
  ...
  node(worker) {
    ...
    stage('Waiting for the app to be ready...') {
      container('tools') {
        sh 'cd tools && ./run.sh http://spring-boot-mysql:8080'
      }
    }
    ...
    stage('Testing...') {
      container('e2e') {
        sh """
          export SUT_URL=http://spring-boot-mysql:8080
